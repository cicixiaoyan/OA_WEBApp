...
.controller('NoticeCtrl', function($scope, $ionicModal, ajaxHelper, $http, $cordovaFileTransfer) { //
    ...
    $scope.downloadFile = function(name) {
        ...
        $http({
                method: 'POST',
                url: "http://192.168.0.70:80/ashx/Notice.ashx/downloadfile",
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                transformRequest: function(obj) {
                ...
                data: { action: "downloadfile", s_fileName: name }
            })
            .success(function(data, status, header, config) {
        ...
        // var url = "http://221.237.158.212:50100/AttDown.aspx?number=201741711848197938.doc&amp;fname=portal/gg";
        // var filename = url.split("/").pop();
        // alert(filename);
        // alert(cordova.file);
        // alert("1" + cordova.file.documentsDirectory);
...
.controller('BacklogCtrl', function($scope, $ionicModal) {
    ...
    $scope.downloadFile = function(pathurl) {
        ...
        var url = "http://221.237.158.212:50100/AttDown.aspx?number=201741711848197938.doc&amp;fname=portal/gg";
        var filename = url.split("/").pop();
        alert(cordova.plugins.Keyboard);
        alert(cordova.file);
        alert("1" + cordova.file.documentsDirectory);
        var targetPath = cordova.file.documentsDirectory + filename;
        var trustHosts = true;
...
/*新建工作详情*/
.controller('NewworkdetailCtrl', function($scope, $stateParams, $ionicPopover) {
    ...
    // 审批选择
    $scope.approvalChoose = {
        "manager": false, //经办人员
        "department": false, //经办部门
        "role": false, //经办角色
        "auditor": false, //审核人员
    };
    ...
    $scope.managerList = [
        { "ui_id": "admin", "ui_desc": "系统管理员" },
        { "ui_id": "admin1", "ui_desc": "系统管理员1" },
        { "ui_id": "admin2", 'ui_desc': "系统管理员2" },
        { "ui_id": "admin3", "ui_desc": "系统管理员3" }
    ];

    $scope.checkManager = function($event) {
        //var managerArray = $scope.managers.ui_id.split(",");
        //判断是否check---------新建可以不用
        // angular.forEach($scope.managerList, function(data, index) {
        //     for (var i in managerArray) {
        //         if (data.ui_id === managerArray[i]) {
        //             $scope.managerList[index].check = true;
        //         }
        //     }
        // });


        $scope.popoverTitle = "经办人员选择";
        $scope.approvalChoose.manager = true;
        $scope.popover.show($event);
    };
    $scope.chooseManager = function($index) {
        $scope.managerList[$index].check = $scope.managerList[$index].check ? false : true;
    };
    ...
    $scope.departmentList = [
        { "bm_mc": "部门", "bmid": "bumen" },
        { "bm_mc": "部门1", "bmid": "bumen1" },
        { "bm_mc": "部门2", "bmid": "bumen2" }
    ];

    $scope.checkDepartment = function($event) {
        $scope.popoverTitle = "经办部门选择";
        $scope.approvalChoose.department = true;
        $scope.popover.show($event);
    };

    $scope.chooseDepartment = function($index) {
        $scope.departmentList[$index].check = $scope.departmentList[$index].check ? false : true;
    };
    ...
    $scope.checkRole = function($event) {
        $scope.popoverTitle = "经办角色选择";
        $scope.approvalChoose.role = true;
        $scope.popover.show($event);
    };

    $scope.chooseRole = function($index) {
        $scope.roleList[$index].check = $scope.roleList[$index].check ? false : true;
    };
    /*
     **审批人员选择
     */
    ...
    // $scope.checkAuditor = function($event) {
    //     $scope.popoverTitle = "审批人员选择";
    //     $scope.approvalChoose.auditor = true;
    //     $scope.popover.show($event);
    // };

    // $scope.chooseAuditor = function($index) {
    //     $scope.auditorList[$index].check = $scope.auditorList[$index].check ? false : true;
    // };
    ...
    $scope.comfirm = function() {
        if ($scope.approvalChoose.manager) {
            $scope.managers = { "ui_id": "", "ui_desc": "" };
            angular.forEach($scope.managerList, function(data, index) {
                if (data.check) {
                    console.log(data)
                    $scope.managers.ui_id += data.ui_desc + ",";
                    $scope.managers.ui_desc += data.ui_desc + ",";

                }
            })
        } else if ($scope.approvalChoose.department) {
            $scope.departments = { "bm_mc": "", "bmid": "" };
            angular.forEach($scope.departmentList, function(data, index) {
                if (data.check) {
                    $scope.departments.bm_mc += data.bm_mc + ",";
                    $scope.departments.bmid += data.bmid + ",";
                }
            })
        } else if ($scope.approvalChoose.role) {
            $scope.roles = { "zwmc": "" };
            angular.forEach($scope.roleList, function(data, index) {
                if (data.check) {
                    $scope.roles.zwmc += data.zwmc + ",";
                }
            })
        }
        // else if ($scope.approvalChoose.auditor) {
        //     $scope.auditors = { "ui_id": "", "ui_desc": "" };
        //     angular.forEach($scope.auditorList, function(data, index) {
        //         if (data.check) {
        //             $scope.auditors.ui_id += data.ui_desc + ",";
        //             $scope.auditors.ui_desc += data.ui_desc + ",";
        //         }
        //     })
        // }
        $scope.popover.hide();
     ...
     *选择经办人员或部门
     */
    $ionicPopover.fromTemplateUrl('templates/Area/newwork-checks.html', {
        scope: $scope
    }).then(function(popover) {
        $scope.popover = popover;
    });
